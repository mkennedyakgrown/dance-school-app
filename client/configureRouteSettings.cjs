...
function invalidConfigTypeAlert(configType) {
  throw new Error(
    "Route configuration failed! Invalid program mode detected!",
    configType
  );
}
...
function updateViteConfig(fs, configType) {
  ...
  switch (configType) {
    case ConfigurationType.DEVELOPMENT:
      ...
      updateLineFunction = (line) => {
        ...
        return (
          ...
          line.substring(commentIndex + COMMENT_MARK.length)
        );
      };
      break;
    case ConfigurationType.PRODUCTION:
      ...
      addCommentColIndex =
        proxyCommentIndex >= 0
          ? dataArr[proxyCommentIndex].indexOf("proxy")
          : null;
      loopCondition = (index) => {
        return (
      ...
      updateLineFunction = (line) => {
        const newLine =
          line.substring(0, addCommentColIndex) +
          COMMENT_MARK +
          line.substring(addCommentColIndex);
  ...
  if (proxyCommentIndex < 0) {
    console.log(
      `Routes are already configured for ${configType.toLowerCase()}!`
    );
    process.exit(0);
  }
